...
def get_model(graph_path,
    ...
    ############ define model ############
    main_input = Input((frames, ROI_N, 2), name='points')
    # main_input = Lambda(lambda x: x[:, :, :, :1])(main_input_)
    # age_input = Lambda(lambda x: x[:, :1, 0, 0])(main_input_)
    ...
    net = LSTM(116, return_sequences=False)(net)
    # net = Lambda(lambda x: tf.concat([x[0], x[1]], axis=1))([net, age_input])
    # net = BatchNormalization()(net)
    net = Dense(1, kernel_regularizer=l1(1e-3))(net)
