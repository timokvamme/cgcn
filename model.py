...
#     """
    ...
    try:
        ...
        idx_ = tf.range(batch_size*time_step) * num_points # [0, 116, 232, ..., batchsize*140]


    idx_ = tf.reshape(idx_, [batch_size, time_step, 1, 1]) # (batchsize, 140, 1, 1)
    ...
    try:
        ...
        point_cloud_flat = tf.reshape(point_cloud_series, [-1, num_dims]) # (batchsize*140*116*5, 1)
        
    # point_cloud_neighbors defined by k-NN
    point_cloud_neighbors = tf.gather(point_cloud_flat, nn_idx+idx_) # (batchsize, 140, 116, 5, 1)
    print("point_cloud_neighbors.shape", point_cloud_neighbors.shape)
    point_cloud_central = tf.expand_dims(point_cloud_central, axis=-2)

    # Copy the central point data for k times
    point_cloud_central = tf.tile(point_cloud_central, [1, 1, 1, k, 1]) # (batchsize, 140, 116, 5, 1)
    ...
    # Therefore, feature * 2
    edge_feature = tf.concat([point_cloud_central, point_cloud_neighbors-point_cloud_central], axis=-1) # (batchsize, 140, 116, 5, 2)
    return edge_feature
